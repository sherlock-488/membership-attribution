...
def find_thresholds_first(scores_first_shadow):
    ...
    y_true = np.concatenate((np.ones(1000), np.zeros(2000)))
...
def find_thresholds_second(scores_MIU):
    ...
    y_true = np.concatenate((np.zeros(1000), np.ones(1000), np.zeros(1000)))
...
def evaluate(scores_first, scores_MIU, threshold_first, threshold_second):
    preds = np.full(len(scores_first), 2)
    ...
    y_true = np.concatenate((np.zeros(1000), np.ones(1000), np.full(1000, 2)))
    accuracy = np.mean(preds == y_true)
    print(f"Evaluation Accuracy: {accuracy}")

    f1 = metrics.f1_score(y_true, preds, average='macro', labels=[0, 1, 2])
    print(f"F1 Score (macro, 3 classes): {f1}")
...
def compute_macro_auc(scores_first, scores_MIU, scores_second):
    ...
    labels_first = np.concatenate((np.ones(1000), np.zeros(2000)))
    labels_second = np.concatenate((np.zeros(1000), np.ones(1000), np.zeros(1000)))
