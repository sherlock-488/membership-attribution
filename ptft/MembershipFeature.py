...
class MembershipFeature():
    ...
    def close_probe(self, model, tokens, query_time=100):
        ...
        for idx in range(1, tokens.input_ids.shape[-1]):
            gt = model.generate(
                tokens.input_ids[:, :idx], max_new_tokens=1,
            ...
            candidates = torch.multinomial(probs, num_samples=query_time).squeeze(1)
    ...
    def prob_diff_weight(self, prob_1, prob_2, label):
        ...
        return self._log_value(prob_2_true)-self._log_value(prob_1_true)
