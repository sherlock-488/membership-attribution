...
SAVE_PATH = '/home/qshi24/scratchycao43/qshi24/MA_results/'
...
# ===== MIMIR：基于 The Pile 的 pretrain MI benchmark =====
MIMIR_DATASET = os.getenv('MIMIR_DATASET', 'iamgroot42/mimir')
# 单 config 默认用 full_pile（通过 MIMIR_CONFIG_LIST 控制）
MIMIR_CONFIG  = os.getenv('MIMIR_CONFIG',  'wikipedia_(en)')
# 默认优先尝试 ngram_7_0.2，不行再回退到 none（在 _load_mimir_sources 里）
...
# ===== WikiMIA：基于 Wikipedia 的 pretrain MI benchmark =====
...
def _load_oloma_sources():
    ...
    Y = Paloma 里 Dolma 100 Subreddits 子语料（allenai/paloma, config=dolma_100_subreddits，val+test）
    ...
    if 'source' not in x.column_names:
        raise ValueError(
            ...
            "无法按 Reddit 子集筛选。请改成 allenai/dolma v1_6-sample 或别的带 source 的 Dolma 版本，"
            "或者在外部预处理好只含 reddit 的数据集再传进来。"
        )

    # 只保留 Dolma 里的 Reddit 子集
...
def _load_mimir_sources():
    """
    抽取 MIMIR 的 member / nonmember：
      - 从 MIMIR_CONFIG_LIST 里的 config 读数据
      - 对每个 config 优先尝试使用 MIMIR_SPLIT（例如 ngram_7_0.2），不行再回退到 'none'
      - 把所有 member 拼成一个大集合 X
      - 把所有 nonmember 拼成一个大集合 W
      - 这里的 shuffle 用固定 seed，这样 target / shadow 看到的是同一套顺序
    """
    config_names = [
    ...
    for cfg in config_names:
        ...
        # 优先尝试你设的 split（例如 ngram_7_0.2），不行就回退到 'none'
    ...
    if not x_parts or not w_parts:
        raise ValueError(
            f"MIMIR: 在 configs={config_names} 中没找到合法的 member/nonmember 数据，"
            "请检查 MIMIR_CONFIG_LIST / MIMIR_SPLIT 设置，或者是否已经开通 gated dataset 权限。"
        )
...
def _mimir_YBZN(x, w, mode: str, group_size: int = MIMIR_GROUP_SIZE):
    """
    通用的 Y/B/Z/N 切分工具（原本为 MIMIR，这里也给 WikiMIA 复用），要求：
      - target 和 shadow 使用 **不重叠** 的样本
...
def load_train_data(dataname, training_mode, training_stage):
    if dataname == 'agnews':
        ...
        # MIMIR：类比 OLOMA，但 target/shadow 使用不重叠的 Y/B/Z/N 段
        if training_mode not in ['target', 'shadow']:
            raise ValueError('Invalid training mode')
        if training_stage not in ['first', 'second']:
            raise ValueError('Invalid training stage')
        ...
        # WikiMIA：完全复用 MIMIR 的 Y/B/Z/N 逻辑
        if training_mode not in ['target', 'shadow']:
            raise ValueError('Invalid training mode')
        if training_stage not in ['first', 'second']:
            raise ValueError('Invalid training stage')
